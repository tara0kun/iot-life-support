...
print('LINE通知送信完了')
"
        # cloudflared が生きている限り待機する（systemd Type=simple 互換）
        # cloudflared が自動更新で SIGTERM を受けて終了したら、ここから抜けて
        # systemd が auto-restart で再実行される
        echo "tunnel monitoring (PID=$TUNNEL_PID)..."
        wait $TUNNEL_PID
        echo "cloudflared exited (probably auto-update or crash) — systemd will restart"
        exit 1   # 終了コード 0 以外で systemd Restart=always が確実に再起動
    fi
    sleep 1
done
