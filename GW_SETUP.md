...
### 自宅で動作確認しておく
```bash
cd ~/iot-life-support && source venv/bin/activate
...
設置先Wi-Fi情報は `.env` の `GRANDMA_WIFI_SSID` / `GRANDMA_WIFI_PASS` に記載済み。
...
2. P110Mをコンセントに差す
3. 炊飯器のプラグをP110Mに差す
4. **TapoアプリでMatter再ペアリング**（設置先Wi-Fiに繋がるよう）
...
@reboot sleep 30 && cd /home/pi/iot-life-support && bash scripts/start_tunnel.sh

# 毎日3時 DBバックアップ
0 3 * * * cd /home/pi/iot-life-support && bash scripts/backup_db.sh

# 5分おき ヘルスチェック
*/5 * * * * cd /home/pi/iot-life-support && bash scripts/health_check.sh

# お薬チェック
0 9,12 * * * cd /home/pi/iot-life-support && venv/bin/python scripts/scheduled_notify.py medicine

# お風呂チェック
0 18 * * * cd /home/pi/iot-life-support && venv/bin/python scripts/scheduled_notify.py bath

# 1日のまとめ
0 22 * * * cd /home/pi/iot-life-support && venv/bin/python scripts/scheduled_notify.py summary

# 家族タスクリマインダー（時刻に担当者へ）
0 7-22 * * * cd /home/pi/iot-life-support && venv/bin/python scripts/scheduled_notify.py care_tasks

# 週次レポート（毎週日曜22時）
0 22 * * 0 cd /home/pi/iot-life-support && venv/bin/python scripts/weekly_report.py

# 異常検知（10分おき）
*/10 * * * * cd /home/pi/iot-life-support && venv/bin/python scripts/anomaly_check.py
```
