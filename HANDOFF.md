...
- **ステータス**: GW投入完了、祖母宅で **全センサー稼働中**（カメラ＋H100ハブ＋T110×4＋T100×1＋SwitchBot温湿度計＋P110M炊飯器）
- **ブランチ**: `dev` で開発継続中（5/4-5/5 で大量コミット未実施）。**コミット未保存の変更が大量にある状態**
- **ネットワーク**: WiFi化完了（IODATA-2G、wlan0=192.168.0.31）。Tailscale引き続き有効
- **顔認識**: 祖母（30件以上）+ 家族D（8件）登録済み、stream1+upsample=2 で動作中。祖父は未登録
- **LINE通知**: マスタースイッチ ON、家族全員にブロードキャスト

### 🆕 5/4-5/5 で実装した主な変更（**未コミット**）
...
- **bath_motion のモンスタークラスター**: 脱衣所モーションが連続発火→1セッションに2000+イベント集約→誤って「夜食」「朝食」ラベル
- **LINE確認待ちセッション 6件** が未対応のまま蓄積（id=105〜114）
- **iot-monitor 再起動が未実施** → session_confirm の新コードが動いていない可能性
- **湿度センサ readings が詳細イベント表を埋めて他が見えない**
- **炊飯器の保温電圧変化単独で通知が発火**してしまうケースが残っている（蓋連動が効いてない場合）
