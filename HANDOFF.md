...
- **顔認識**: 祖母・家族D登録、受動収集 60px 閾値に調整済（5/14 までは80px でカット）
...
- 顔登録: 祖母・家族D のみ。祖父・家族A・家族B・家族C・家族E は person 登録あるが encoding 未紐付（必要なら帰省時に登録）
...
| **データ整理** | seed 「母」を「家族A」に統合・削除、5/3朝の保温パルス誤検知データクリーンアップ |
...
- **SwitchBot 防水温湿度計**: BLE 直接読取り、MAC=<.env の SWITCHBOT_METER_MAC>
...
- `HUB_IP`
- `FAMILY_PASSWORD`（値は .env のみ。ドキュメントに書かないこと）/ `TABLET_TOKEN`
...
**炊飯器の電力監視は死んでいなかった。** P110M は Matter 経由
(`sensors.matter_plug`)で、TPAP とは無関係。故障期間中も毎日
朝夕に記録が残っている(故障期間中も毎日)。
一時「最終 09/14」と見えたのは、単にその後炊飯していなかっただけ。
**電力監視は稼働時(100W 超え)しか記録しない。**

死んでいたのは H100 ハブ配下の **T110 4 個と T100 1 個だけ**
(冷蔵庫 / 浴室ドア / トイレドア / 炊飯器の蓋 / 脱衣所モーション)。

なお故障期間中も炊飯は毎日されていたのに食事セッションは 09/05 で
止まっている。判定が冷蔵庫・蓋・カメラの組み合わせを要求する作りで、
**電力だけでは成立しない**ため。
...
## 🌿 ブランチの状態(2026-09-15 時点)

    main                            本番。dev と一致
    dev                             作業用。main と一致
    future                          **停止中**(最終 2026-04-27)
    claude/family-manual-docs-XjPnO **停止中**(最終 2026-05-04)

### 運用で踏んだ落とし穴

**PR でマージしたあと `dev` を追従させていなかった。** そのため
`main` にマージコミットが 7 件溜まり、`dev` から見ると 7 件遅れている
状態が続いていた(中身の差はゼロだが、`git log main..dev` が読めなくなる)。

    マージ後は必ず: git checkout dev && git merge --ff-only origin/main && git push

### 停止中の 2 本について

**どちらも捨ててはいけない。** 実装が入っている。

`future`(main より +6 / -80、競合 3 箇所)

- 祖母用タブレットのキオスク APK(`android/main.py` + buildozer)
- GitHub Actions で APK 自動ビルド
- 家族UIの週間サマリー
- タブレットの Service Worker(オフライン対応)
- 複数 LINE 通知先の基盤、データアーカイブ、エラーフォールバック

`claude/family-manual-docs-XjPnO`(main より +3 / -80、競合 5 箇所)

- 家族用マニュアルの全面改訂 + PDF 版
- **リポジトリ全体の匿名化**(個人特定情報・機材型番の一般化)

**匿名化は公開範囲に関わる。** このリポジトリは公開されており、
`HANDOFF.md` に Tailscale Funnel の URL と内部 IP が載っている
(家族UIは認証で保護されているので直ちに危険ではないが、意図した
公開かどうかは確認したほうがよい)。

### 取り込むときの注意

4〜5 か月分の乖離があるため、機械的なマージは危険。競合の解決には
「4 か月前にどちらが正しかったか」の判断が要る。**当時の意図を知って
いる人が行うこと。**

---

## 🛁 2026-09-15 お風呂が毎日「未入浴」扱いになっていた

センサー復旧後の全体点検で見つかった、**11 日間の故障とは無関係な積年のバグ**。

### 症状

直近 20 日すべてで、実際には入浴しているのに 18 時に
「祖母がまだお風呂に入っていないようです」が送られていた
(`bath_reminder` 累計 74 件)。

### 原因

3 段の噛み合わせ:

1. `sessions.py` はお風呂セッションを `confirmed=0`(未確定)で作る
2. `monitor.py` の `_request_session_confirmation()` は
   「お風呂は bath_classification 系で別途確認するため」LINE 確認を
   **意図的にスキップ**する → `confirmed=1` になる経路が存在しない
3. `sessions_today()` の既定は **`confirmed=1` のみ**

結果、お風呂セッション 461 件のうち **452 件(98%)が永久に未確定**となり、
集計から丸ごと消えていた。2 の設計判断と 3 の既定値が噛み合っていない。

影響は 3 箇所:

| 箇所 | 症状 |
| --- | --- |
| `scheduled_notify.check_bath()` | 毎日 18 時に誤リマインド |
| `scheduled_notify.daily_summary()` | 22 時のお花スコアでお風呂が永久に未実施 |
| `line_commands._bath_info()` | 「お風呂: まだ」固定表示 |

### 同時に見つかった: 家族の手動記録が埋もれる

`web/app.py` の手動記録 INSERT 4 箇所が `confirmed` を指定しておらず、
既定の 0 で入っていた。**`family_report` 由来 37 件のうち確定は 10 件だけ**。
タブレットで「記録しました」と表示されても、食事回数にもお花にも
反映されない状態だった。

### 直し方

- `sessions_today()` の既定クエリを
  `(confirmed = 1 OR (label = 'お風呂' AND confirmed <> -1))` に変更。
  お風呂は**家族が明示的に却下(-1)しない限り実績として数える**。
  確認フロー自体を通らないので、却下されていない = 実績、が正しい。
  食事側の意味は変えていない(未確定の食事は従来どおり数えない)。
- 手動記録 INSERT 4 箇所に `confirmed=1, confirmed_by='family_ui'` を追加。
  **家族の入力そのものが確認行為**なので、確認待ちにする理由がない。
- `scripts/test_session_confirm.py` を追加(DB 不要、実装から SQL を
  抜き出して検証)。修正前のコードに当てると FAIL することを確認済み。

### 残っている宿題

- **過去データは未確定のまま**。お風呂 452 件 / 家族の手動記録 27 件は
  `confirmed=0` で残っている。本番 DB への backfill は未実施。
- **食事セッションの 54% が未確定**(60 日で 確定 115 / 未確定 187 / 却下 39)。
  これは別問題で、`recheck_pending` の自動確定(`b1af658`)が効くのは
  今後の分のみ。過去分は残る。
- `session_aggregator` だけ `supervise()` を通していない裸の `create_task`。
  例外で落ちると食事・お風呂の記録が全部止まるのに誰も気づかない。
