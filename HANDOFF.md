...
- **ステータス**: GW投入完了、自宅から遠隔運用フェーズ。誤検知抑制・LINE操作整備・受動学習を実装済み
- **ブランチ**: `dev` 4コミット先行（5/4-5/8 セッションで実装）、main から計 **48コミット先行**
- **ネットワーク**: WiFi（IODATA-2G、wlan0=192.168.0.31）+ Tailscale (100.123.131.127) で遠隔SSH可
- **顔認識**: 祖母 + 家族D登録済み、受動収集システム稼働中（祖父等の未識別顔は遠隔ラベル付け予定）
- **LINE通知**: マスタースイッチ ON、家族全員にブロードキャスト + メニュー/意見ボックス機能
