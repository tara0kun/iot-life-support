...
**ステータス**: 祖母宅で **全主要センサー稼働開始**（GW投入完了）。**運用しながら誤検知の調整中**
**ブランチ**: `dev` で運用中／`main` は 5/2朝時点で停止
**未コミット変更**: **大量にあり**（5/4-5/5 のセッションでコード20+ファイル修正、未保存）
...
- **顔認識**: 祖母（30件以上）+ 家族D（8件）登録、stream1+upsample=2 で動作。祖父は未登録
- **未設置**: T110（歯ブラシ/シャンプー）、T100（洗面所）、S200B、ドライヤーP110M

### 🆕 5/4-5/5 セッション主な変更（**未コミット**）
...
### 5/5 時点の登録家族
- person_id=1: 祖母 / 3: 祖父 / 4: 家族A / 5: 家族B / 6: 家族C / 7: 家族D
...
**運用上の誤検知問題**:
- [ ] **bath_motion のモンスタークラスター対策**: 脱衣所モーションが連続発火→1セッションに2000+イベント集約→誤って「夜食」「朝食」ラベル
  - `_qualifies_as_session` で bath_motion 単独 or 過剰イベント数を除外
- [ ] **炊飯器の保温電圧変化のみで通知発火**問題: 蓋連動を確実に
  - `_ask_rice_action_classification` 内で「直近X分以内に蓋開なし→通知も出さない」を厳格化
- [ ] **家族UI 詳細イベント表で湿度センサ readings が他を埋もれさせる**
  - 大幅変化のみ表示、または開閉センサ反応と同時の場合のみ表示
- [ ] **夜1時〜朝5時の通知抑制**（トイレ以外）
- [ ] **既存の confirmed=0 セッション 6件**（id=105〜114）クリーンアップ
- [ ] **iot-monitor 再起動**で session_confirm の新コードを有効化

**判定ロジック見直し**:
- [ ] **トイレの判定方法すり合わせ**（現状: T110 open/close そのまま記録）
- [ ] **食事行動真偽のすり合わせ**（誤検知パターンの洗い出し）
- [ ] **学習の状況確認 UI**（rice_classifications, bath_classifications の蓄積件数・精度）
...
- [ ] センサ反応・判定根拠の可視化（家族UI）
  - 「不明」回答数、「他の家族」回答数
  - 各センサーの直近24h反応回数
  - 通知の判定根拠を message に含める

**カメラ**:
- [ ] 祖父さんの顔登録（カメラ識別精度向上のため）
...
- [ ] **LINE リッチメニュー作成**:
  ...
  - 使い方ガイドへのリンク
- [ ] **使い方ガイド作成**（マニュアル一式）:
  - タブレット セットアップ編（画面ピン留め手順詳細）
  - 祖母自身の使い方編（イラスト多め）
  - 家族用セットアップ編
  - 機能リファレンス編
- [ ] **本セッション全変更のコミット** + main へのマージ
- [ ] S200B ボタン 2台のコード対応
...
1. **家族の LINE 自己登録**（「登録 （あなたの名前）」のような自由名で family_line_users に追加）
