...
- [ ] **「状況」コマンドの情報拡充**（小: 30分）
  - トイレ回数、お風呂時間、家族が記録した証言も含める
  - LINE1通で1日の全情報が把握できる状態にする
- [ ] **異常検知の能動通知**（中: 1〜2h）
  - センサーが一定時間無反応 → 「倒れている可能性」アラート
  - 深夜(2時〜5時)の炊飯器稼働 → 緊急通知
  - 冷蔵庫が30分以上開きっぱなし → 通知
- [ ] **家族間の役割分担通知**（中: 2h）
  - 「今日の薬チェック担当: 母」のようにLINEで朝にアサイン
  - 対応した家族が「済」と返信すると全員に共有
- [ ] **週次レポート自動生成**（中〜大: 2〜3h）
  - 週1で食事回数・達成率・お薬確認率を集計
  - LINEで送付、可能ならPDF化してメール送信
...
- [ ] **音声読み上げ**
  - タブレットのメッセージをWeb Speech APIで読み上げ
  - 祖母が画面を見ていなくても通知に気づける
- [ ] **祖母の声の感情分析（遠大）**
