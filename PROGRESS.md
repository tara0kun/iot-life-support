...
### 炊飯量ガイドのDB管理化
- [x] `rice_guide` テーブル追加（meal, amount）
- [x] 家族UIに「炊飯量ガイド」設定セクション追加（朝食/昼食/夕食ごとに登録・削除）
...
### 🔴 優先度高
- [ ] **「できた」ボタンのクールダウン** — 同じボタンを30分間押せないようにする（認知症の連打防止）
- [ ] **`/api/tablet-record` の認証** — 外部公開時にトークンチェックがない。悪意あるAPI呼び出しを防ぐ

### 🟡 優先度中
- [ ] **Python 3.12+ datetime非推奨警告の対応** — SQLite `detect_types=PARSE_DECLTYPES` の代替
- [ ] **セッション集約の整合性** — `meal_sessions` への直接INSERTと `aggregate_sessions()` の混在を解消
- [ ] **1日のまとめ通知に祖母ボタン検証状況を含める** — 「お薬ボタン押下→未確認」等
- [ ] **家族UIリアルタイム更新の日付対応** — 過去日閲覧時にリロードが走る問題

### 🟢 優先度低（運用開始後）
- [ ] **タブレットのオフライン対応** — Service Workerで最後のキャッシュ表示（Wi-Fi切断時の白画面防止）
- [ ] **週間ダッシュボード** — 食事回数推移グラフ、スタンプ達成率のトレンド
- [ ] **複数LINE通知先** — 母と孫で通知レベルを分離（緊急は全員、日常は母だけ等）
- [ ] **古いデータのアーカイブ** — 数ヶ月運用後のクエリ速度対策
- [ ] **エラー時フォールバック** — Matterサーバ/カメラ落ち時にWeb UIが巻き込まれない保護
...
25. **`/api/tablet-record` は認証なし**（外部公開時のセキュリティ課題、GW前に要対応）
