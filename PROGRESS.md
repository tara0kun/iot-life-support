...
**ステータス**: 祖母宅セットアップ中（GW中、5/2〜5/6）
**ブランチ**: `dev`で運用中（祖母宅Pi も dev で稼働、main マージは GW投入後）／`main` 5/2朝時点で停止／`future` は実験機能保持
**最新コミット**: 詳細は `git log --oneline` 参照（5/2-5/3 で 24コミット追加）

### 🏠 祖母宅でのセットアップ状況（5/3時点）
- **稼働済**: ラズパイ5（有線LAN 192.168.0.23、Tailscale）/ iot-matter / iot-web / iot-monitor / Cloudflare Tunnel / LINE
- **稼働済 P110M**: 炊飯器コンセントに設置、Matter経由で電力監視
- **未設置**: H100×2 / T110×6 / T100×3 / C220 / S200B×2 / タブレット / SwitchBot温湿度計（未購入）

### 🆕 5/2-5/3 で実装した主な機能
1. **家族の LINE 自己登録**（「登録 （あなたの名前）」のような自由名で family_line_users に追加）
2. **未確定セッションの人物確認**（Quick Reply で家族が割当、近接統合）
3. **全アラートの actionable 化**（「✓ 確認した」ボタン → 全員ブロードキャスト「☑️ ○○さんが対応」 → 30分応答無しで再通知 → タイムアウト）
...
7. **SwitchBot 防水温湿度計のコード準備**（センサー到着・bleak導入時に有効化）

### 5/3 時点の登録家族
- person_id=1: 祖母 / 3: 祖父 / 4: 家族A / 5: 家族B / 6: 家族C
- 旧 person_id=2「母」は 4「家族A」と統合・削除
...
### 🔴 高優先度
- [ ] **物理センサー設置**（H100, T110×6, T100×3, C220, タブレット）
  - Tapoアプリで祖母宅Wi-Fiに接続
  - エイリアス設定（炊飯器/冷蔵庫/浴室ドア/脱衣所/トイレ/歯ブラシ/シャンプー）
  - `.env` の HUB_IP / CAMERA_IP を 192.168.0.x の新IPに更新
  - `sudo systemctl restart iot-monitor`
- [ ] 顔登録（祖母・祖父）C220設置後
- [ ] 全センサー統合テスト
- [ ] cron 登録（DBバックアップ・ヘルスチェック・recheck_pending・各種定期通知）

### 🟡 中優先度
- [ ] P110M ② をドライヤー転用、`.env` で `HAIR_DRYER_NODE_ID` 設定
- [ ] SwitchBot 防水温湿度計購入＋bleakインストール＋設定
